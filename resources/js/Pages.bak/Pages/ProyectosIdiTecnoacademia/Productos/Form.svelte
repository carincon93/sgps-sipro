...
<form on:submit|preventDefault={submit}>
    <fieldset class="p-8" disabled={proyectoIdiTecnoacademia.allowed.to_update ? undefined : true}>
        <div class="mt-8">
            <Select
                id="tipo_productos_linea_66_id"
                items={tiposProductos}
                bind:selectedValue={$form.tipo_productos_linea_66_id}
                error={errors.tipo_productos_linea_66_id}
                autocomplete="off"
        ...
        <div class="mt-8">
            ...
            <File
                id="soporte"
                maxSize="10000"
                bind:value={$form.soporte}
                valueDb={producto?.soporte}
                error={errors.soporte}
                route={producto?.soporte?.includes('http') || method == 'store'
                    ? null
                    : route('proyectos-idi-tecnoacademia.productos.download-file-sharepoint', [proyectoIdiTecnoacademia, producto, 'soporte'])} />
        </div>
