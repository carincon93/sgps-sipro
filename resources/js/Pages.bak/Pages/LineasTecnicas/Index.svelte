            ...
            {#each lineasTecnicas.data as lineaTecnica (lineaTecnica.id)}
                <tr class="hover:bg-gray-100 focus-within:bg-gray-100">
                    ...
                    <td class="border-t td-actions">
                        <DataTableMenu class={lineasTecnicas.data.length < 3 ? 'z-50' : ''}>
                            <Item on:SMUI:action={() => Inertia.visit(route('lineas-tecnicas.edit', lineaTecnica.id))} disabled={!lineaTecnica.allowed.to_view} class={!lineaTecnica.allowed.to_view ? 'hidden' : ''}>
                                <Text>Ver detalles</Text>
                            </Item>

                            <Separator class={!lineaTecnica.allowed.to_destroy ? 'hidden' : ''} />
                            <Item on:SMUI:action={() => ((lineaTecnicaId = lineaTecnica.id), (dialogEliminar = true), (allowedToDestroy = lineaTecnica.allowed.to_destroy))} disabled={!lineaTecnica.allowed.to_destroy} class={!lineaTecnica.allowed.to_destroy ? 'hidden' : ''}>
                                <Text>Eliminar</Text>
                            </Item>
                        </DataTableMenu>
                    </td>
