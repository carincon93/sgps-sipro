...
import { Breadcrumbs, Chip, Divider, Grid, MenuItem, Tab, TableCell, TableRow, Tabs } from '@mui/material'
...
const Index = ({ auth, semilleros_investigacion, redes_conocimiento, allowed_to_create }) => {
    ...
    const [lineas_investigacion_filtradas, setLineasInvestigacionFiltradas] = useState([])
    const filterLineasInvestigacion = (semillero_investigacion) => {
        setLineasInvestigacionFiltradas(
            semillero_investigacion?.linea_investigacion?.grupo_investigacion?.lineas_investigacion.map((item) => {
                return {
                    value: item.id,
                    label: item.nombre,
                }
                                ...
                                <MenuMui text={<MoreVertIcon />}>
                                    {semillero_investigacion.id !== semillero_investigacion_to_destroy ? (
                                        <div>
                                            <MenuItem
                                                onClick={() => (
                                                    setDialogStatus(true), setMethod('PUT'), setSemilleroInvestigacion(semillero_investigacion), filterLineasInvestigacion(semillero_investigacion)
                                                )}
