...
<form on:submit|preventDefault={handleSubmit} class="mt-20">
    {#if user}
        <InfoMessage>
            El usuario con número de documento {user.numero_documento} ya está registrado en el sistema. Por favor inicie sesión haciendo <a use:inertia href="/" class="underline">clic aquí</a>. Si no recuerda la contraseña la puede recuperar en el siguiente enlace <a use:inertia href="/forgot-password" class="underline">Recuperar contraseña</a>
        </InfoMessage>
    {/if}
    <div>
        ...
        {#if errors.email == 'El campo correo electrónico ya ha sido tomado.'}
            <InfoMessage alertMsg={true} class="mt-4">
                <p class="mt-3 py-4 text-justify">
                    El correo electrónico ya fue registrado, Por favor inicie sesión haciendo <a use:inertia href="/" class="underline">clic aquí</a>. Si no recuerda la contraseña la puede recuperar en el siguiente enlace <a use:inertia href="/forgot-password" class="underline">Recuperar contraseña</a>
                </p>
            </InfoMessage>
        {/if}
    </div>
    ...
    <div class="flex items-center justify-end mt-4">
        ...
        <LoadingButton loading={$form.processing} type="submit" bind:disabled={$form.autorizacion_datos}>Continuar</LoadingButton>
    </div>
</form>
