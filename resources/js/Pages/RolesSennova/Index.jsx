<script>
    ...
    function destroy() {
        if (allowedToDestroy) {
            Inertia.delete(route('roles-sennova.destroy', rolSennovaId), {
                preserveScroll: true,
                onFinish: () => {
            ...
            {#each rolesSennova.data as rolSennova (rolSennova.id)}
                <tr className="hover:bg-gray-100 focus-within:bg-gray-100">
                    ...
                    <td className="border-t td-actions">
                        <DataTableMenu className={rolesSennova.data.length < 3 ? 'z-50' : ''}>
                            <Item on:SMUI:action={() => Inertia.visit(route('roles-sennova.edit', rolSennova.id))} disabled={!is_super_admin} className={!is_super_admin ? 'hidden' : ''}>
                                <Text>Ver detalles</Text>
                            </Item>

                            <Separator className={!is_super_admin ? 'hidden' : ''} />
                            <Item on:SMUI:action={() => ((rolSennovaId = rolSennova.id), (dialogEliminar = true), (allowedToDestroy = is_super_admin))} disabled={!is_super_admin} className={!is_super_admin ? 'hidden' : ''}>
                                <Text>Eliminar</Text>
                            </Item>
                        </DataTableMenu>
                    </td>
