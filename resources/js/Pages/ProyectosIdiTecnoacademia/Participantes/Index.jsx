...
<AuthenticatedLayout>
        ...
        <div className="flex items-center justify-between lg:px-8 max-w-7xl mx-auto px-4 py-6 sm:px-6">
            <div>
                <h1>
                    ...
                    <span className="text-app-400 font-medium">/</span>
                    <a use:inertia href={route('proyectos-idi-tecnoacademia.participantes.index', proyectoIdiTecnoacademia.id)} className="text-app-400 hover:text-app-600 font-extrabold underline">Participantes</a>
                    <span className="text-app-400 font-medium">/</span>
    ...
    <a use:inertia href={route('proyectos-idi-tecnoacademia.productos.index', proyectoIdiTecnoacademia.id)} className="flex bottom-0 fixed hover:bg-app-600 mb-4 px-6 py-2 bg-app-700 rounded-lg shadow-2xl text-center text-white z-50">
            ...
            <tbody>
                {#each proyectoIdiTecnoacademia.participantes as participante (participante.id)}
                    <tr className="hover:bg-gray-100 focus-within:bg-gray-100">
                        ...
                        <td className="border-t td-actions">
                            <DataTableMenu className={proyectoIdiTecnoacademia.participantes.length < 3 ? 'z-50' : ''}>
                                <Item on:SMUI:action={() => showParticipante(participante)} disabled={!proyectoIdiTecnoacademia.allowed.to_update} className={proyectoIdiTecnoacademia.allowed.to_update ? '' : 'hidden'}>
                                    <Text>Editar</Text>
                                </Item>
    ...
    <Dialog bind:open={dialogOpen} id="participante">
        ...
        <div slot="content">
            <form on:submit|preventDefault={submitParticipante} id="participante-form">
                <fieldset disabled={isSuperAdmin || checkRole(authUser, [5, 10, 12, 22]) ? undefined : true}>
                    ...
                    <div className="mt-8">
                        <Input label="Número de meses de vinculación al proyecto" id="cantidad_meses" type="number" input$step="0.1" input$min="1" input$max="11.5" className="mt-1" bind:value={$formParticipante.cantidad_meses} placeholder="Número de meses de vinculación" autocomplete="off" required />
                    </div>
                    <div className="mt-8">
        ...
        <div slot="actions" className="flex w-full">
            ...
            <PrimaryButton loading={$formParticipante.processing} className="ml-auto" type="submit" form={formID}>Vincular</PrimaryButton>
        </div>
    </Dialog>
    ...
    <Dialog bind:open={openNuevoParticipanteDialog} id="nuevo-participante">
        ...
        <div slot="content">
            <form on:submit|preventDefault={submitRegister} id={formNuevoIntegranteId}>
                <fieldset>
                    <div className="mt-8">
                        <Input label="Nombre completo" id="nombre_nuevo_participante" type="text" className="mt-1" bind:value={$formNuevoIntegrante.nombre} error={errors.nombre} required />
                    </div>

                    <div className="mt-8">
                        <Input label="Correo electrónico institucional" id="email_nuevo_participante" type="email" className="mt-1" bind:value={$formNuevoIntegrante.email} error={errors.email} required />
                    </div>
                    ...
                    <div className="mt-8">
                        <Input label="Número de documento" id="numero_documento_nuevo_participante" type="number" input$min="55555" input$max="9999999999999" className="mt-1" bind:value={$formNuevoIntegrante.numero_documento} error={errors.numero_documento} required />
                    </div>

                    <div className="mt-8">
                        <Input label="Número de celular" id="numero_celular_nuevo_participante" type="number" input$min="3000000000" input$max="9999999999" className="mt-1" bind:value={$formNuevoIntegrante.numero_celular} error={errors.numero_celular} required />
                    </div>
