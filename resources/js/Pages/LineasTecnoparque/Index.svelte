            ...
            {#each lineasTecnoparque.data as lineaTecnoparque (lineaTecnoparque.id)}
                <tr class="hover:bg-gray-100 focus-within:bg-gray-100">
                    ...
                    <td class="border-t td-actions">
                        <DataTableMenu class={lineasTecnoparque.data.length < 3 ? 'z-50' : ''}>
                            <Item on:SMUI:action={() => Inertia.visit(route('lineas-tecnoparque.edit', lineaTecnoparque.id))} disabled={!isSuperAdmin} class={!isSuperAdmin ? 'hidden' : ''}>
                                <Text>Ver detalles</Text>
                            </Item>

                            <Separator class={!isSuperAdmin ? 'hidden' : ''} />
                            <Item on:SMUI:action={() => ((lineaTecnoparqueId = lineaTecnoparque.id), (dialogEliminar = true), (allowedToDestroy = isSuperAdmin))} disabled={!isSuperAdmin} class={!isSuperAdmin ? 'hidden' : ''}>
                                <Text>Eliminar</Text>
                            </Item>
                        </DataTableMenu>
                    </td>
