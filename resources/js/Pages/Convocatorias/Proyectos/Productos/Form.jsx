...
<form on:submit|preventDefault={submit}>
    <fieldset className="p-8" disabled={proyecto.allowed.to_update ? undefined : true}>
        ...
        <div className="mt-8 mb-8">
            <Label className="text-center" required value="Fecha de ejecución" />
        ...
        {#if proyecto.codigo_linea_programatica != 70 || isSuperAdmin}
            <div className="mt-8">
                <Label required className="mb-4" labelFor="resultado_id" value="Resultado" />
            ...
            <div className="mt-8">
                ...
                {#if proyecto.codigo_linea_programatica != 70}
                    {#if $form.tatp_servicio_tecnologico == true}
                        <InfoMessage className="my-8" message="Deber ser medible y con una fórmula. Por ejemplo: (# metodologías validadas/# metodologías totales) X 100" />
                    {:else}
                        <InfoMessage className="my-8" message="Especifique los medios de verificación para validar los logros del proyecto." />
                    {/if}
                {/if}
        ...
        {#if $form.tatp_servicio_tecnologico == false}
            <div className="mt-8">
                <Label required className="mb-4" labelFor="subtipologia_minciencias_id" value="Subtipología Minciencias" />
            ...
            <div className="mt-8">
                ...
                {#if proyecto.codigo_linea_programatica != 70}
                    {#if proyecto.servicio_tecnologico}
                        <InfoMessage className="my-8" message="Los medios de verificación corresponden a las evidencias y/o fuentes de información en las que está disponibles los registros, la información necesaria y suficiente. Dichos medios pueden ser documentos oficiales, informes, evaluaciones, encuestas, documentos o reportes internos que genera el proyecto, entre otros." />
                    {:else}
                        <InfoMessage className="my-8" message="Especifique los medios de verificación para validar los logros del objetivo específico." />
                    {/if}
                {/if}
        ...
        <div className="bg-white rounded shadow overflow-hidden">
            ...
            <div className={actividades?.length > 0 ? 'grid grid-cols-2' : ''}>
                {#if actividades?.length > 0}
                    {#each actividades as { id, descripcion }, i}
                        <Label className="p-3 border-t border-b flex items-center text-sm" labelFor={'linea-tecnologica-' + id} value={descripcion} />

                        <div className="border-b border-t flex items-center justify-center">
                            <input type="checkbox" bind:group={$form.actividad_id} id={'linea-tecnologica-' + id} value={id} className="rounded text-app-500" />
                        </div>
                    {/each}
                {:else}
