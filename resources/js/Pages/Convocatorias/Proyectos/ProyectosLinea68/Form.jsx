            ...
            {#each servicioTecnologico.proyecto.evaluaciones as evaluacion, i}
                {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                    <div className="bg-zinc-900 p-4 rounded shadow text-white my-2">
                        ...
                        <p className="whitespace-pre-line text-xs">{evaluacion.servicio_tecnologico_evaluacion.titulo_comentario ? evaluacion.servicio_tecnologico_evaluacion.titulo_comentario : 'Sin recomendación'}</p>
                    </div>
                {/if}
            {/each}
...
<div className="py-24">
    ...
    <div className="mt-4 flex items-start justify-around">
        <div className="mt-4 flex {errors.fecha_inicio ? '' : 'items-center'}">
            ...
            <div className="ml-4">
                <input id="fecha_inicio" type="date" className="mt-1 block w-full p-4" min={convocatoria.min_fecha_inicio_proyectos_linea_68} max={convocatoria.max_fecha_finalizacion_proyectos_linea_68} bind:value={$form.fecha_inicio} required disabled={evaluacion ? 'disabled' : undefined} />
            </div>
        </div>
        <div className="mt-4 flex {errors.fecha_finalizacion ? '' : 'items-center'}">
            ...
            <div className="ml-4">
                <input id="fecha_finalizacion" type="date" className="mt-1 block w-full p-4" min={convocatoria.min_fecha_inicio_proyectos_linea_68} max={convocatoria.max_fecha_finalizacion_proyectos_linea_68} bind:value={$form.fecha_finalizacion} required disabled={evaluacion ? 'disabled' : undefined} />
            </div>
        </div>
    </div>
        ...
        <RecomendacionEvaluador className="mt-8">
            {#each servicioTecnologico.proyecto.evaluaciones as evaluacion, i}
                {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                    <div className="bg-zinc-900 p-4 rounded shadow text-white my-2">
                        <p className="text-xs">Evaluador COD-{evaluacion.id}:</p>
                        <p className="whitespace-pre-line text-xs">{evaluacion.servicio_tecnologico_evaluacion.fecha_ejecucion_comentario ? evaluacion.servicio_tecnologico_evaluacion.fecha_ejecucion_comentario : 'Sin recomendación'}</p>
                    </div>
                {/if}
            {/each}
            {#if servicioTecnologico.proyecto.evaluaciones.length == 0}
...
<fieldset className="py-24" disabled>
    <div className="grid grid-cols-2">
        <div>
            <Label required disabled={evaluacion ? 'disabled' : undefined} className="mb-4" labelFor="tipo_proyecto_st_id" value="Centro de formación" />
        </div>
        <div>
...
<div className="py-24">
    <div className="grid grid-cols-2">
        ...
        <div>
            <Input label="Nombre del área técnica" id="nombre_area_tecnica" type="text" className="mt-1" error={errors.nombre_area_tecnica} bind:value={$form.nombre_area_tecnica} required disabled={evaluacion ? 'disabled' : undefined} />
        </div>
    </div>
</div>
...
<div className="py-24">
    <h1 className="text-2xl text-center" id="estructura-proyecto">Estructura del proyecto</h1>
            ...
            <RecomendacionEvaluador className="mt-8">
                {#each servicioTecnologico.proyecto.evaluaciones as evaluacion, i}
                    {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                        <div className="bg-zinc-900 p-4 rounded shadow text-white my-2">
                            <p className="text-xs">Evaluador COD-{evaluacion.id}:</p>
                            <p className="whitespace-pre-line text-xs">{evaluacion.servicio_tecnologico_evaluacion.resumen_comentario ? evaluacion.servicio_tecnologico_evaluacion.resumen_comentario : 'Sin recomendación'}</p>
                        </div>
                    {/if}
                {/each}
            </RecomendacionEvaluador>
        ...
        <RecomendacionEvaluador className="mt-8">
            {#each servicioTecnologico.proyecto.evaluaciones as evaluacion, i}
                {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                    <div className="bg-zinc-900 p-4 rounded shadow text-white my-2">
                        <p className="text-xs">Evaluador COD-{evaluacion.id}:</p>
                        <p className="whitespace-pre-line text-xs">{evaluacion.servicio_tecnologico_evaluacion.antecedentes_comentario ? evaluacion.servicio_tecnologico_evaluacion.antecedentes_comentario : 'Sin recomendación'}</p>
                    </div>
                {/if}
            {/each}
            {#if servicioTecnologico.proyecto.evaluaciones.length == 0}
        ...
        <RecomendacionEvaluador className="mt-8">
            {#each servicioTecnologico.proyecto.evaluaciones as evaluacion, i}
                {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                    <div className="bg-zinc-900 p-4 rounded shadow text-white my-2">
                        <p className="text-xs">Evaluador COD-{evaluacion.id}:</p>
                        <p className="whitespace-pre-line text-xs">{evaluacion.servicio_tecnologico_evaluacion.identificacion_problema_comentario ? evaluacion.servicio_tecnologico_evaluacion.identificacion_problema_comentario : 'Sin recomendación'}</p>
                    </div>
                {/if}
            {/each}
            {#if servicioTecnologico.proyecto.evaluaciones.length == 0}
        ...
        <RecomendacionEvaluador className="mt-8">
            {#each servicioTecnologico.proyecto.evaluaciones as evaluacion, i}
                {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                    <div className="bg-zinc-900 p-4 rounded shadow text-white my-2">
                        <p className="text-xs">Evaluador COD-{evaluacion.id}:</p>
                        <p className="whitespace-pre-line text-xs">{evaluacion.servicio_tecnologico_evaluacion.pregunta_formulacion_problema_comentario ? evaluacion.servicio_tecnologico_evaluacion.pregunta_formulacion_problema_comentario : 'Sin recomendación'}</p>
                    </div>
                {/if}
            {/each}
            {#if servicioTecnologico.proyecto.evaluaciones.length == 0}
        ...
        <RecomendacionEvaluador className="mt-8">
            {#each servicioTecnologico.proyecto.evaluaciones as evaluacion, i}
                {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                    <div className="bg-zinc-900 p-4 rounded shadow text-white my-2">
                        <p className="text-xs">Evaluador COD-{evaluacion.id}:</p>
                        <p className="whitespace-pre-line text-xs">{evaluacion.servicio_tecnologico_evaluacion.justificacion_problema_comentario ? evaluacion.servicio_tecnologico_evaluacion.justificacion_problema_comentario : 'Sin recomendación'}</p>
                    </div>
                {/if}
            {/each}
            {#if servicioTecnologico.proyecto.evaluaciones.length == 0}
...
<div className="py-24">
    <div className="grid grid-cols-2">
        <div>
            <Label required disabled={evaluacion ? 'disabled' : undefined} className="mb-4" labelFor="programas_formacion" value="Nombre de los programas de formación con los que se relaciona el proyecto" />
        </div>
        <div>
...
<div className="py-24">
    <div className="grid grid-cols-2">
        ...
        <div>
            <Input label="Zona de influencia" id="zona_influencia" type="text" className="mt-1" error={errors.zona_influencia} placeholder="Escriba el número de aprendices que se beneficiarán en la ejecución del proyecto" bind:value={$form.zona_influencia} required disabled={evaluacion ? 'disabled' : undefined} />
        </div>
    </div>
</div>
        ...
        <RecomendacionEvaluador className="mt-8">
            {#each servicioTecnologico.proyecto.evaluaciones as evaluacion, i}
                {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                    <div className="bg-zinc-900 p-4 rounded shadow text-white my-2">
                        <p className="text-xs">Evaluador COD-{evaluacion.id}:</p>
                        <p className="whitespace-pre-line text-xs">{evaluacion.servicio_tecnologico_evaluacion.bibliografia_comentario ? evaluacion.servicio_tecnologico_evaluacion.bibliografia_comentario : 'Sin recomendación'}</p>
                    </div>
                {/if}
            {/each}
            {#if servicioTecnologico.proyecto.evaluaciones.length == 0}
