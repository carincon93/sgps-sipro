...
const Index = ({ auth, convocatoria, proyectos_linea_68, allowed_to_create }) => {
    ...
    const [proyecto_linea_68_to_destroy, setProyectoLinea68ToDestroy] = useState(null)
        ...
        <AuthenticatedLayout header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Líneas programáticas</h2>}>
            <Grid container>
                {checkRole(auth_user, [1, 20, 18, 19, 5, 17]) && (
                    <Grid item md={12}>
                        <TabsConvocatoria value="0" convocatoria={convocatoria} linea_programatica_id="10" />
                    </Grid>
                )}

                <Grid item md={12} className="!mt-20">
                    <h1 className="text-3xl text-center">Fortalecimiento de la oferta de servicios tecnológicos para las empresas - Línea 68</h1>
                </Grid>
                ...
                <Grid item md={12}>
                    ...
                    <TableMui rows={['Título', 'Fecha de ejecución', 'Estado (Evaluación)', 'Acciones']} sxCellThead={{ width: '320px' }}>
                        {allowed_to_create && (
                            <TableRow
                                onClick={() => router.visit(route('convocatorias.proyectos-linea-68.create', [convocatoria.id]))}
                                variant="raised"
                            ...
                            <TableRow key={id}>
                                ...
                                <TableCell>
                                    <MenuMui text={<MoreVertIcon />}>
                                        {proyecto.id !== proyecto_linea_68_to_destroy ? (
                                            <div>
                                                <MenuItem
                                                ...
                                                {proyecto.evaluaciones.map((evaluacion, i) => (
                                                    <MenuItem
                                                        key={i}
                                                        onClick={() => router.visit(route('convocatorias.proyectos-linea-68.edit', [convocatoria.id, id, { evaluacion_id: evaluacion?.id }]))}>
                    ...
                    <PaginationMui links={proyectos_linea_68.links} className="mt-6" />
                </Grid>
            </Grid>
        </AuthenticatedLayout>
    )
