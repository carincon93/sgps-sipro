...
const Index = ({ auth, convocatoria, proyectos_formulario_15_linea_65, allowed_to_create }) => {
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Lista de proyectos - Formulario 15 Línea 65" />

            <Grid container>
                {checkRole(auth_user, [1, 20, 18, 19, 5, 17]) && (
                    <Grid item md={12}>
                        <TabsConvocatoria value="0" convocatoria={convocatoria} tipo_formulario_convocatoria_id="15" />
                    </Grid>
                )}

                <Grid item md={12} className="!mt-20">
                    <h1 className="text-3xl text-center">Formulario 15: Sello editorial SENA - Apropiación de la ciencia y la tecnología y cultura de la innovación y la competitividad - Línea 65</h1>
                </Grid>
                ...
                <Grid item md={12}>
                    ...
                    <TableMui rows={['Título', 'Fecha de ejecución', 'Estado (Evaluación)', 'Acciones']} sxCellThead={{ width: '320px' }}>
                        {allowed_to_create && (
                            <TableRow
                                onClick={() => router.visit(route('convocatorias.proyectos-formulario-15-linea-65.create', [convocatoria.id]))}
                                variant="raised"
                        ...
                        {proyectos_formulario_15_linea_65.data.map(({ id, proyecto, titulo, fecha_ejecucion }) => (
                            <TableRow key={id}>
                                <TableCell>
                                    <div>
                                ...
                                <TableCell>
                                    ...
                                    (convocatoria.fase == 5 && proyecto?.mostrar_recomendaciones && convocatoria.tipo_convocatoria == 3) ? (
                                        <>
                                            <AlertMui>
                                                {proyecto?.estado_evaluacion_proyecto_formulario_15_linea_65?.estado}
                                                <div>Puntaje: {proyecto?.estado_evaluacion_proyecto_formulario_15_linea_65?.puntaje}</div>
                                                <small>
                                                    Número de recomendaciones: {proyecto?.estado_evaluacion_proyecto_formulario_15_linea_65?.numero_recomendaciones}
                                                    <br />
                                                    Evaluaciones: {proyecto?.estado_evaluacion_proyecto_formulario_15_linea_65?.evaluaciones_habilitadas} habilitada(s) /{' '}
                                                    {proyecto?.estado_evaluacion_proyecto_formulario_15_linea_65?.evaluaciones_finalizadas} finalizada(s)
                                                </small>
                                            </AlertMui>
                                    ...
                                    {is_super_admin || checkRole(auth_user, [18]) ? (
                                        <>
                                            {proyecto?.estado_evaluacion_proyecto_formulario_15_linea_65?.alerta && (
                                                <AlertMui severity="error" className="mt-4">
                                                    Importante: {proyecto?.estado_evaluacion_proyecto_formulario_15_linea_65?.alerta}
                                                </AlertMui>
                                            )}
                                        </>
                                    ) : null}
                                    ...
                                    <MenuMui text={<MoreVertIcon />} className="">
                                        {proyecto.id !== proyecto_linea_65_to_destroy ? (
                                            <div>
                                                ...
                                                <MenuItem>
                                                    <a href={route(`convocatorias.proyectos.pdf-formulario15-linea65`, [convocatoria.id, proyecto.id])} target="_blank" className="flex items-center">
                                                        PDF del proyecto
                                                    </a>
                                                </MenuItem>
                                                <Divider />
                                                ...
                                                <MenuItem
                                                    ...
                                                    onClick={(e) => {
                                                        ...
                                                        if (proyecto.allowed.to_update) {
                                                            router.delete(route('convocatorias.proyectos-formulario-15-linea-65.destroy', [convocatoria.id, proyecto.id]), {
                                                                preserveScroll: true,
                                                            })
                                                        }
                                                    }}>
                    ...
                    <PaginationMui links={proyectos_formulario_15_linea_65.links} className="mt-6" />
                </Grid>
            </Grid>
        </AuthenticatedLayout>
    )
