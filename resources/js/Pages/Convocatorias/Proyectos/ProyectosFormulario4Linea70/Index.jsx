...
const Index = ({ auth, convocatoria, proyectos_formulario_4_linea_70, allowed_to_create }) => {
        ...
        <AuthenticatedLayout header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Líneas programáticas</h2>}>
            <Grid container>
                {checkRole(auth_user, [1, 20, 18, 19, 5, 17]) && (
                    <Grid item md={12}>
                        <TabsConvocatoria value="0" convocatoria={convocatoria} tipo_formulario_convocatoria_id="4" />
                    </Grid>
                )}
                ...
                <Grid item md={12}>
                    ...
                    <TableMui rows={['TecnoAcademia', 'Fecha de ejecución', 'Estado (Evaluación)', 'Acciones']} sxCellThead={{ width: '320px' }}>
                        ...
                        {proyectos_formulario_4_linea_70.data.map(({ id, proyecto, titulo, fecha_ejecucion, proyecto_base }) => (
                            <TableRow key={id}>
                                <TableCell>
                                    <div>
                                ...
                                <TableCell>
                                    {is_super_admin ||
                                    checkRole(auth_user, [5]) ||
                                    (checkRole(auth_user, [4, 22, 12]) && proyecto?.mostrar_recomendaciones) ||
                                    (checkRole(auth_user, [4, 22, 12]) && proyecto?.mostrar_recomendaciones) ||
                                    (convocatoria.fase == 5 && proyecto?.mostrar_recomendaciones) ? (
                                        <>
                                            <AlertMui>
                                                {proyecto?.estado_evaluacion_proyecto_formulario_4_linea_70?.estado}
                                                <small>
                                                    Número de recomendaciones: {proyecto?.estado_evaluacion_proyecto_formulario_4_linea_70?.numero_recomendaciones}
                                                    <br />
                                                    Evaluaciones: {proyecto?.estado_evaluacion_proyecto_formulario_4_linea_70?.evaluaciones_habilitadas} habilitada(s) /{' '}
                                                    {proyecto?.estado_evaluacion_proyecto_formulario_4_linea_70?.evaluaciones_finalizadas} finalizada(s)
                                                </small>
                                            </AlertMui>
                                    ...
                                    {is_super_admin || checkRole(auth_user, [18]) ? (
                                        <>
                                            {proyecto?.estado_evaluacion_proyecto_formulario_4_linea_70?.alerta && (
                                                <AlertMui severity="error" className="mt-4">
                                                    Importante: {proyecto?.estado_evaluacion_proyecto_formulario_4_linea_70?.alerta}
                                                </AlertMui>
                                            )}
                                        </>
                                    ) : null}
                                ...
                                <TableCell>
                                    <MenuMui text={<MoreVertIcon />}>
                                        {proyecto.id !== proyecto_formulario_4_linea_70_to_destroy ? (
                                            <div>
                                                <MenuItem
                                                    onClick={() => router.visit(route('convocatorias.proyectos-formulario-4-linea-70.edit', [convocatoria.id, id]))}
                                                ...
                                                {proyecto.evaluaciones.map((evaluacion, i) => (
                                                    <MenuItem
                                                        key={i}
                                                        onClick={() =>
                                                            router.visit(route('convocatorias.proyectos-formulario-4-linea-70.edit', [convocatoria.id, id, { evaluacion_id: evaluacion?.id }]))
                                                        }
                                                ...
                                                <MenuItem
                                                    ...
                                                    onClick={(e) => {
                                                        ...
                                                        if (proyecto.allowed.to_update) {
                                                            router.delete(route('convocatorias.proyectos-formulario-4-linea-70.destroy', [convocatoria.id, proyecto.id]), {
                                                                preserveScroll: true,
                                                            })
                                                        }
                                                    }}>
                    ...
                    <PaginationMui links={proyectos_formulario_4_linea_70.links} className="mt-6" />
                </Grid>
            </Grid>
        </AuthenticatedLayout>
    )
