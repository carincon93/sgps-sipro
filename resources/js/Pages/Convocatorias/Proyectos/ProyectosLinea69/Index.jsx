...
const Index = ({ auth, convocatoria, proyectos_linea_69, allowed_to_create }) => {
    ...
    const [proyecto_linea_69_to_destroy, setProyectoLinea69ToDestroy] = useState(null)
        ...
        <AuthenticatedLayout header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Líneas programáticas</h2>}>
            <Grid container>
                {checkRole(auth_user, [1, 20, 18, 19, 5, 17]) && (
                    <Grid item md={12}>
                        <TabsConvocatoria value="0" convocatoria={convocatoria} linea_programatica_id="4" />
                    </Grid>
                )}
                ...
                <Grid item md={12}>
                    ...
                    <TableMui rows={['Nodo Tecnoparque', 'Fecha de ejecución', 'Estado (Evaluación)', 'Acciones']} sxCellThead={{ width: '320px' }}>
                        {allowed_to_create && (
                            <TableRow
                                onClick={() => router.visit(route('convocatorias.proyectos-linea-69.create', [convocatoria.id]))}
                                variant="raised"
                            ...
                            <TableRow key={id}>
                                ...
                                <TableCell>
                                    <MenuMui text={<MoreVertIcon />}>
                                        {proyecto.id !== proyecto_linea_69_to_destroy ? (
                                            <div>
                                                <MenuItem
                                                ...
                                                {proyecto.evaluaciones.map((evaluacion, i) => (
                                                    <MenuItem
                                                        key={i}
                                                        onClick={() => router.visit(route('convocatorias.proyectos-linea-69.edit', [convocatoria.id, id, { evaluacion_id: evaluacion?.id }]))}>
                    ...
                    <PaginationMui links={proyectos_linea_69.links} className="mt-6" />
                </Grid>
            </Grid>
        </AuthenticatedLayout>
    )
