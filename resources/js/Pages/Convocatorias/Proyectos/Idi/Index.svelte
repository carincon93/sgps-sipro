        ...
        <thead slot="thead">
            <tr class="text-left font-bold">
                ...
                <th class="px-6 pt-6 pb-4 sticky top-0 z-10 bg-white shadow-xl w-full">
                    Estado (Evaluación) {#if convocatoria.tipo_convocatoria == 2}(No aplica para proyectos demo){/if}
                </th>
        ...
        <tbody slot="tbody">
            {#each idi.data as { id, proyecto, titulo, fecha_ejecucion }}
                <tr class="hover:bg-gray-100 focus-within:bg-gray-100">
                    ...
                    <td class="border-t">
                        {#if isSuperAdmin || checkRole( authUser, [18], ) || (checkRole(authUser, [4, 6]) && proyecto?.mostrar_recomendaciones && convocatoria.tipo_convocatoria == 1) || (checkRole(authUser, [4, 6]) && proyecto?.mostrar_recomendaciones && convocatoria.tipo_convocatoria == 3) || (convocatoria.fase == 5 && proyecto?.mostrar_recomendaciones && convocatoria.tipo_convocatoria == 1) || (convocatoria.fase == 5 && proyecto?.mostrar_recomendaciones && convocatoria.tipo_convocatoria == 3)}
                            <p class="px-6 py-4">
                                {proyecto?.estado_evaluacion_idi?.estado}
                                <small>
                                    Puntaje: {proyecto?.estado_evaluacion_idi?.puntaje}
                                </small>
                                {#if isSuperAdmin || checkRole(authUser, [18])}
                                    <br />
                                    <small>
                    ...
                    <td class="border-t td-actions">
                        <DataTableMenu class={idi.data.length < 3 ? 'z-50' : ''}>
                            <Item on:SMUI:action={() => Inertia.visit(route('convocatorias.idi.edit', [convocatoria.id, id]))} disabled={!proyecto?.allowed?.to_view} class={!proyecto?.allowed?.to_view ? 'hidden' : ''}>
