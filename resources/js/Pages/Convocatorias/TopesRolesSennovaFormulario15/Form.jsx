...
const Form = ({ method = '', setDialogStatus, convocatoria, tope_rol_sennova_formulario_15, centros_formacion, roles_sennova, ...props }) => {
    const form = useForm({
        centro_formacion_id: tope_rol_sennova_formulario_15?.centro_formacion_id,
        convocatoria_rol_sennova_id: tope_rol_sennova_formulario_15?.convocatoria_rol_sennova_id,
        cantidad_maxima: tope_rol_sennova_formulario_15?.cantidad_maxima,
        meses_maximos: tope_rol_sennova_formulario_15?.meses_maximos,
    })

    const submit = (e) => {
        e.preventDefault()
        method == 'POST'
            ? form.post(route('convocatorias.topes-roles-sennova-formulario-15.store', [convocatoria.id]), {
                  onSuccess: () => setDialogStatus(false),
                  preserveScroll: true,
              })
            : form.put(route('convocatorias.topes-roles-sennova-formulario-15.update', [convocatoria.id, tope_rol_sennova_formulario_15.id]), {
                  onSuccess: () => setDialogStatus(false),
                  preserveScroll: true,
              })
    }
